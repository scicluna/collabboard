...
function useDragAndZoom({ initialZoom = 1, dragToolActive = true }: UseDragAndZoomProps) {
    ...
    function dragMouseDown(e: React.MouseEvent) {
        if ((e.target as HTMLElement).classList.contains('note') || (e.target as HTMLElement).classList.contains('pin') || (e.target as HTMLElement).classList.contains('line') || (e.target as HTMLElement).classList.contains('image')) return;
